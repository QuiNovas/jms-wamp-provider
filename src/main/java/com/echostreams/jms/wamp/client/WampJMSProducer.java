...
import com.echostreams.jms.wamp.utils.MessageConverterUtils;
...
public class WampJMSProducer implements JMSProducer {
    private static final Logger LOGGER = LoggerFactory.getLogger(WampJMSProducer.class);
    ...
    public WampJMSProducer() {

    }

    public WampJMSProducer(WampJMSContext jmsContext) throws JMSException {
    ...
    public JMSProducer send(Destination destination, Message message) {
        try {
            ...
            throw WampJMSException.createRuntimeException(e);
        }
        return this;
    }
    ...
    public JMSProducer setProperty(String name, boolean value) {
        try {
            setPropertyInternal(name, Boolean.valueOf(value));
        } catch (JMSException e) {
            throw WampJMSException.createRuntimeException(e);
        }
        return this;
    ...
    private void sendMessage(Destination destination, Message message) throws JMSException {
        ...
        try {
            ...
            Topic topic = (Topic) replyTo;
            PublishOptions publishOptions = new PublishOptions(true, false);
            // Publish on topic with args and publishOptions
            CompletableFuture<Publication> completableFuture = jmsContext.getWampSession().
                    publish(topic.getTopicName(), publishOptions, message); //ObjectSerializer().objectToByteArray(message)
            completableFuture.thenAccept(publication -> {
                try {
                    LOGGER.info("Published on " + topic.getTopicName());
                } catch (JMSException e) {
